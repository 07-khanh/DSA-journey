...
### $O$-notation
$$\begin{align*}O(g(n)) = \{f(n)&: \text{there exist positive constants } c \text{ and } n_0 \text{ such that }\\  
&0 \le f(n) \le c\cdot g(n) \text{ for all } n \ge n_0\}\end{align*}$$
...
Set up the inequality:$$5n^2 - 2n + 10 \le c \cdot n^2$$
To find $c$, we can ignore the negative terms and overestimate the constants:$$5n^2 - 2n + 10 \le 5n^2 + 10$$
For $n \ge 1$, we know $10 \le 10n^2$, so:$$5n^2 + 10 \le 5n^2 + 10n^2 = 15n^2$$
...
_We want to subtract a term from $2n^3$ but still keep it proportional to $n^3$_.
...
$$\Rightarrow F_i = \left \lfloor \dfrac{\phi^i}{\sqrt{5}} + \dfrac{1}{2}\right \rfloor = \left \lfloor \dfrac{\phi^i}{\sqrt{5}} \right \rceil$$
...
However, when using induction, it is not necessary to show base case in out proof because for any constant $n$, $T(n)$ is always constant, and it is always possible to choose base cases that work.
...
## Master Method
Used for many Divide-and-Conquer Recurrence with the form $$T(n) = aT(n/b) + f(n)$$ where $a \ge 1, b > 1$ and $f(n) > 0$.
...
_($f(n)$ is polynomially smaller than $n^{\log_ba}$)_.
**Solution:** $$T(n) = \Theta(n^{\log_ba})$$
Intuitively: Cost dominated by leaves.

**Case 2**: $f(n) = \Theta(n^{\log_ba} \cdot \lg^kn)$, where $k \ge 0$.
_($f(n)$ is within polylog factor of $n^{\log_ba}$ but not smaller)_.

**Solution**: $$T(n) = \Theta(n^{\log_ba}\cdot \lg^{k+1} n)$$
...
_($f(n)$ is polynomially greater than $n^{\log_ba}$)_.
**Solution**: $$T(n)=(f(n))$$
